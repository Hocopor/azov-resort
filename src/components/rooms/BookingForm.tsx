...
export function BookingForm({
  ...
  const [showRegisterOffer, setShowRegisterOffer] = useState<{bookingId: string, email: string} | null>(null)
  ...
  const onSubmit = async (data: FormData) => {
    ...
    if (!session && !data.agreedToTerms) {
      showError('Для продолжения необходимо принять политику обработки персональных данных, пользовательское соглашение и условия бронирования.')
      return
    }
    ...
    try {
      ...
      if (!session) {
        setShowRegisterOffer({ bookingId: result.bookingId, email: data.guestEmail || '' })
      } else {
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      ...
      {step === 'form' && (
        <div className="space-y-5">
          <div className="rounded-xl border border-sea-100 bg-sea-50 px-3 py-2">
            <button
              type="button"
              onClick={() => setStep('calendar')}
              className="mb-1 text-sm font-medium text-sea-700 hover:underline inline-block"
            >
              ← Изменить даты
            </button>
            <div className="text-sm font-semibold text-sea-700 mb-2">
              {formatDate(range!.from!)} — {formatDate(range!.to!)} · {stayDaysLabel(nights)}
            </div>
            
            <div className="border-t border-sea-200 pt-2 space-y-1">
              <div className="text-lg font-bold text-gray-900">
                Итого {formatMoney(totalPrice)}
              </div>
              <div className="text-sm font-semibold text-sea-700">
                Депозит {formatMoney(depositAmount)}
              </div>
            </div>
          </div>
          ...
          <div className="rounded-2xl border border-sand-200 bg-sand-100 p-4 text-sm text-gray-600">
            ...
            <p>
              При бронировании оплачивается депозит <strong>{formatMoney(depositAmount)}</strong>. Оставшаяся сумма{' '}
          ...
          {!session && (
            <label className="flex items-start gap-3 cursor-pointer select-none">
              <input type="checkbox" {...register('agreedToTerms')} className="mt-1 h-4 w-4 rounded accent-sea-700" />
              <span className="text-sm text-gray-600">
                Я соглашаюсь с{' '}
                <a href="/legal/privacy" target="_blank" className="text-sea-600 hover:underline">политикой персональных данных</a>,{' '}
                <a href="/legal/terms" target="_blank" className="text-sea-600 hover:underline">пользовательским соглашением</a> и{' '}
                <a href="/legal/booking" target="_blank" className="text-sea-600 hover:underline">условиями бронирования</a>.
              </span>
            </label>
          )}
      ...
      {showRegisterOffer && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 p-4 backdrop-blur-sm">
          <div className="w-full max-w-md rounded-3xl bg-white p-6 shadow-2xl">
            <h3 className="mb-2 text-xl font-bold text-gray-900">Бронь успешно создана! 🎉</h3>
            <p className="mb-6 text-sm text-gray-600">
              Хотите создать аккаунт, чтобы отслеживать статус бронирования и управлять им в личном кабинете?
            </p>
            <div className="flex flex-col gap-3">
              <button
                type="button"
                onClick={() => {
                  router.push(`/auth/register?bookingId=${showRegisterOffer.bookingId}&email=${encodeURIComponent(showRegisterOffer.email)}`)
                }}
              ...
              <button
                type="button"
                onClick={() => {
                  setShowRegisterOffer(null)
                  success('Бронь успешно оформлена!')
                  router.push('/')
                }}
                className="btn-outline w-full justify-center border-gray-200 text-gray-600 hover:bg-gray-50"
              >
                Нет, спасибо
              </button>
            </div>
