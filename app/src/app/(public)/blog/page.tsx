      ...
      <section className="bg-gradient-to-br from-sea-800 to-deep-900 text-white pt-32 pb-16">
        <div className="max-w-2xl mx-auto px-4 text-center">
          <div className="inline-flex items-center gap-2 px-4 py-2 bg-white/15 rounded-full text-sm mb-5">
            <Waves className="w-4 h-4" /> Обстановка онлайн
          </div>
          <h1 className="font-display text-5xl md:text-6xl font-bold mb-4">Что сейчас у моря</h1>
          <p className="text-white/80 text-lg">
            Актуальная информация о погоде, море и жизни на базе отдыха
          </p>
        </div>
      </section>
        ...
        <div className="max-w-2xl mx-auto px-4 space-y-8">
          {posts.length === 0 && (
            <div className="text-center py-20">
              ...
              <p className="text-gray-500">Публикаций пока нет. Скоро появятся!</p>
            </div>
          )}
          ...
          {posts.map((post) => {
            ...
            return (
              <article key={post.id} className="card overflow-hidden">
                  ...
                  <div key={idx}>
                    ...
                    {item.type === 'video' && item.url && (
                      <div>
                        ...
                        />
                        {item.caption && (
                          <p className="text-sm text-gray-500 px-5 pt-2">{item.caption}</p>
                        )}
                      </div>
                    )}
                ...
                <div className="p-5">
                  {post.title && (
                    <h2 className="font-display text-xl font-semibold text-gray-900 mb-2">{post.title}</h2>
                  )}
                  {post.content && (
