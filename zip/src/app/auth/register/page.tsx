...
function RegisterForm() {
  ...
  const defaultBookingId = params?.get('bookingId') || undefined
  ...
  const { register, handleSubmit, formState: { errors } } = useForm<FormData>({
    ...
    defaultValues: { agreeTerms: false, email: defaultEmail },
  })
  ...
  const onSubmit = async (data: FormData) => {
    ...
    try {
      const res = await fetch('/api/auth/register', {
        ...
        body: JSON.stringify({ name: data.name, email: data.email, password: data.password, bookingId: defaultBookingId }),
      })
